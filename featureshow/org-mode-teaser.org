...
- move elements
  - ~M-UP/DOWN~ *move up/down*
  - ~M-LEFT~    *promote*
  - ~M-RIGHT~   *demote*
  - ~M-S-LEFT/RIGHT/UP/DOWN~ move with subitems
...
- ~C-c C-x o~   toggle ORDERED flag for father
- ~C-c ,~       set priority
- ~S-UP/DOWN~   priority up/down
...
- task dependencies with org-depend ([[http://orgmode.org/worg/org-contrib/org-depend.html][docu]])
  ...
  - ~:TRIGGER: chain-silbings(NEXT)~  define trigger for setting NEXT
